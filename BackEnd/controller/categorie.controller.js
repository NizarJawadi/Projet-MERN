...
const { nomC, typeC } = req.body;
const newCategorie = new Categorie({ nomC:nomC, typeC:typeC })
try {
...
const cat1 = {nomC:nomC, typeC:typeC, _id:id };
await Categorie.findByIdAndUpdate(id, cat1);
res.json(cat1);
}
export const deleteCategorie = async (req, res) => {
const { id } = req.params;
if (!mongoose.Types.ObjectId.isValid(id)) return res.status(404).send(`pas de Categories avec l'ID: ${id}`);
